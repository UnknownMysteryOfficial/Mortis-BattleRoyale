...
public class LobbyBrSetCommand implements TabExecutor {

    private final MortisBattleRoyale plugin;

    public LobbyBrSetCommand(MortisBattleRoyale plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] args) {
        ...
        String lobbySetBr = ChatColor.translateAlternateColorCodes('&', msgConfig.getString("lobby-set-br") + " ");

        if (commandSender instanceof Player){
            Player player = (Player) commandSender;
            if (player.hasPermission("mortis.battleroyale.setlobbybr")){
                Location location = player.getLocation();
                config.set("battleroyale.location", location);
                try {
                    config.save(file);
                    player.sendMessage(lobbySetBr);
                } catch (IOException e) {
